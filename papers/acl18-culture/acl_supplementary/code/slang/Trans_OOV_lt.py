...
cnt = 0
lines = open('/home/asd/cdminer/naacl/eval/trans_chinesefinal_0211.txt').read().split('\n')
for line in lines[:]:
    ls = line.split()
    if(len(ls)==151):
...
def ScoreEn(en,title):
    return 1-spatial.distance.cosine(cn_we[title],en_we[en])
...
for line in open('ZH_OOV.tsv','r').read().split('\n'):
    ...
    if(len(ls)!=7):
        ...
        cnt += 1
# the last one is actullly the strong baseline
