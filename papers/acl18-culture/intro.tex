...
%	(What are the most similar English words to ``浮云''?)}
\end{enumerate}
%\vspace{5pt}
...
\begin{figure}[t]
	\centering
	\epsfig{file=figures/intro.pdf, width=1\columnwidth}
	\caption{Two social media messages about Nagoya from different cultures in 2012 }
	\label{fig:intro}
\end{figure}

The first task (\secref{sec:mcdne}) is to mine cross-cultural differences in
...
\figref{fig:intro} illustrates two example microblog messages about Nagoya in Twitter and Weibo respectively.
...
The second task (\secref{sec:bleis}) is to find similar terms for slang across cultures and languages. 
...
%now becomes a common nickname of Chinese governments.
...
%	 (\secref{sec:mcdne} and \secref{sec:bleis})
	
%	\item We release a prototype tool for the proposed approach,
%	two datasets on the above tasks, and several resources, which could potentially benefit future research in cross-cultural social studies and social media analysis.
\end{enumerate}
