...
\subsubsection{Baselines} We compare LPAF as well as its three ablated versions that remove each of the three steps in \secref{sec:ptf} against four categories of methods with a perceivable reduction in model size and computation.
...
\begin{table}[t]
	...
	\caption{Percentage of parameters and FLOPs. 
		%		$T_{sparse}$ has identical parameters and FLOPs as BERT-base because the pruned weights still participate in computation and storage.
	}
	\label{table:stats}
\end{table}
...
%		\hline
		...
		\hline
%\midrule
%		\multicolumn{8}{c}{\textbf{Pre-training Distillation}}   \\
%		\hline
%\midrule
		...
		\hline
%	\midrule
%		\multicolumn{8}{c}{\textbf{Task-specific Distillation}}   \\
%		\hline
		...
		\hline
%		\multicolumn{8}{c}{\textbf{Structured Pruning}}   \\
%		\hline	\midrule
%	\midrule
...
%		\hline
%		\multicolumn{8}{c}{\textbf{Matrix Factorization}}   \\
		\hline
%	\midrule
...
\paragraph{Effect of sparsity-aware SVD}
...
%		\hline
\toprule
		& \multicolumn{3}{c}{Before Step-3 $\rightarrow$ After Step-3}               \\ 
		\midrule
		Weighting Strategy & \multicolumn{1}{c}{$k$=260} & \multicolumn{1}{c}{$k$=130} & \multicolumn{1}{c}{$k$=130} \\ 
		\midrule
		...
		\multicolumn{1}{c|} {vanilla SVD} & \multicolumn{1}{l}{79.1 $\rightarrow$ 91.4} & \multicolumn{1}{l}{77.9 $\rightarrow$ 89.2} & 75.9 $\rightarrow$ 88.8 \\ 
%		\hline
\bottomrule
	\end{tabular}
