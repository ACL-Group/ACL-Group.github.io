...
\begin{table}[]
    ...
    \begin{tabular}{p{0.95\columnwidth}}
        ...
        % Cited & Rules \\
        % \hline
        % Sec 39 (2) of Family Proceeding Act 1980 & \textit{{\color{red}The ground for the order} is established in law if, and only if, the court is satisfied that the parties to {\color{green}the marriage or civil union are living apart, and have been living apart for the period of 2 years} immediately preceding the filing of the application for an order dissolving the marriage or civil union; and no proof of any other matter shall be required to establish the ground.}\\
        % \hline
        % Sec 249 (1) of Crimes Act 1961 & \textit{Every one is liable to {\color{red}imprisonment for a term not exceeding 7 years} {\color{blue}who, directly or indirectly, accesses any computer system and thereby, dishonestly or by deception, and without claim of right},— {\color{green}(a) obtains any property, privilege, service, pecuniary advantage, benefit, or valuable consideration; or (b) causes loss to any other person}.}\\
...
\begin{table}
    ...
    \begin{tabular}{p{0.1\columnwidth}p{0.3\columnwidth}p{0.4\columnwidth}}
    \toprule
...
We employ a message-passing mechanism that aggregates information from source nodes (i.e., nodes without incoming edges) to $\mathcal{V}_g$ in the \lawgraph{}. For instance, source nodes include $\{D, M_t, M_p, M_i, A_l, A_n, M_o, K_{Di}\}$ in Figure~\ref{fig:law-graph-parser-structure}. %Appendix~\ref{sec:a} provides source nodes and output nodes example in the real \lawgraph{}.
...
% The ``OR'' node aggregates from the most significant information from one or multiple neighbors, while an ``AND'' node necessitates the aggregation of all head nodes. 
% \Kaiqi{Give motivation first..} 
Equations~\ref{eq:or} and \ref{eq:and} delineate the operations for ``OR'' and ``AND'' logic nodes, respectively.
% \KZ{User align or array to align these eqns.}
\begin{align}
