...
\cut{Note that while almost all Wikipedia
...
In this paper, \cut{we will show that wikification using link co-occurrence has
major advantages over the previous attempts.}
...
{\scriptsize \tt
\textsf{
Modern portfolio theory(MPT) is a theory of {\uline{investment}} which attempts to maximize portfolio expected {\uline{rate of return}} for a given amount of portfolio risk, or equivalently minimize {\uline{financial risk}} for a given level of expected return, by carefully choosing the proportions of various {\uline{asset}}s.}
}
\end{minipage}
}
\vspace*{2ex}
...
%{\scriptsize \tt
%\textsf{
%{\uline{Modern portfolio theory}}[Modern portfolio theory] ({\uline{MPT}}[Modern portfolio theory]) is a {\uline{theory}}[Theory] of {\uline{investment}} which attempts to maximize {\uline{portfolio}}[Portfolio] expected {\uline{rate of return}} for a given {\uline{amount}}[Quantity] of {\uline{portfolio}}[Portfolio] {\uline{risk}}[Risk], or equivalently minimize {\uline{financial risk}} for a given {\uline{level}}[Level] of {\uline{expected return}}[Expected return], by carefully choosing the {\uline{proportions}}[Proportionality] of various {\uline{asset}}s.}
%}
%\end{minipage}
%}
\fbox{
...
{\scriptsize \tt
\textsf{
{\uline{Modern portfolio theory}}({\uline{MPT}}) is a {\uline{theory}} of {\uline{investment}} which attempts to maximize {\uline{portfolio}} expected {\uline{rate of return}} for a given {\uline{amount}} of {\uline{portfolio}} {\uline{risk}}, or equivalently minimize {\uline{financial risk}} for a given {\uline{level}} of {\uline{expected return}}, by carefully choosing the {\uline{proportions}} of various {\uline{asset}}s.}
}
\end{minipage}
}
\scriptsize
...
{\scriptsize \tt
{\renewcommand\baselinestretch{1.0}\selectfont
The original band, \uline{Polar Bear}[{Snow Patrol}], was formed in 1994 by \uline{Gary Lightbody}[{Gary Lightbody}]
who was a \uline{student}[{Student}] at \uline{Dundee University}[{University of Dundee}] in \uline{Scotland}[{Scotland}].
\par}
}
\end{minipage}
}
\caption{Wikification of Plain Text in Example \ref{ex-bear2} in the Web Demo.
(Concepts are enclosed in square brackets)}
\label{fig:screen-bear}
\end{figure}
