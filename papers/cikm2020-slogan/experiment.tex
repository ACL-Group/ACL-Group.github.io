...
%For the aspect hint selection, we first sample 10 most popular fine-grained categories associated with each topic and manually select 
%most typical categories from them as aspect hints.
...
%\label{sec:baselines}
According to \secref{sec:preference},
...
For human evaluation, instances in HUMtest are distributed to three human annotators with no prior knowedge about the systems. We require them to independently score the slogans on each criteria, ranging from 1 to 5, then average the scores of each criteria as human evaluation results. The inter-annotator agreement of each criteria shown in \tabref{tab:inner_agreement} is computed as the average Pearson or Spearman correlation between every two annotators. The inner-agreement on interestingness, which is more subjective, is not as much satisfied as other criterias.
% higher inner-agreement Interestingness is a more subjective criteria which is difficult to achieve a relatively higher inner-agreement.
...
%	\small
	...
	\begin{tabular}{lcccc}
		\hline
		Model & \begin{tabular}[c]{@{}l@{}}Overall\\quality\end{tabular} & Relevancy &  Fluency & Interestingness \\
		\hline
...
%		\hline\hline
		SSG & \textbf{4.16}  & \textbf{4.32}  & \textbf{4.53}  &  \textbf{4.43}  \\
%		SALE+PLM & -  & -   &  - &   -  \\
		%		SingleSG$_{\mathrm{concept}}$ & 28.34 &  3.30 & 3.32 & 4.31 & 4.38 \\
		%		SingleSG$_{\mathrm{items}}$& 41.11 & 3.84 & 4.0 & 4.30 & 4.22  \\
		%		MultiSG-{biattn} & 36.99 & 3.86 & 4.05 & 4.17 & 4.11 \\
		%		MultiSG-{cat} & 40.59 & 3.95 & 4.13 & 4.34 & 4.23  \\
		GOLD & 4.60 & 4.63 & 4.91 & 4.69  \\
		\hline 
	\end{tabular}
\end{table}
...
%as discussed in \secref{sec:preference}.
...
%Besides, our proposed \emph{is-a} knowledge-aware model SSG generate higher quality slogans
...
%				\scriptsize
		%		\small
		\subfloat[Examples of relation identification in our method.]{
		...
		\subfloat[Comparision of capturing potential user needs.]{
			...
			\begin{tabular}{c|c}
				...
				\begin{tabular}{p{12em}<{\centering}}SSG\end{tabular}	
...
by comparing slogans generated by SSG and Two-CAT in \tabref{tab:semantics}.
...
\tabref{tab:semantics_b} compares slogans generated by SSG and Two-CAT
