...
def CalcAll():
    ...
    SocialSpace_cn = preprocessing.normalize(SocialSpace_cn,norm='l1')

    SocialSpace_en = np.array(SocialSpace_en)
    SocialSpace_en = preprocessing.scale(SocialSpace_en)
    SocialSpace_en = preprocessing.normalize(SocialSpace_en,norm='l1')
    ...
    for line in open('./eval.set','r').read().split('\n'):
        ...
        if(len(ls)==3):
            ...
            final.append( (line, -spatial.distance.cityblock(cur_cn,cur_en) ) )
