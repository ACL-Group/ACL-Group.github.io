...
\begin{figure}[th]
	\centerline{\epsfig{figure=context_bean_flat.eps,width=\columnwidth}}
...
{\em Text context} is the surrounding plain text of both the image
...
As \figref{fig:context-bean} shows, the image context contains limited amount
...
%The source page of an image may contain noises such as
...
%Specifically, we traverse the Document Object Model(DOM)
%tree to extract plain
%texts from the web pages in a depth-first order,
%and mark the position of all images in the plain text.
%We index each keyword to these positions instead of the original images.
%The query context can also be indexed in the same way.
%During the online process, we decompose the query term into a set
%of keywords which are indexed to a number of positions in web pages.
%Then we retrieve the concepts around the position within a window as
%the text context. Finally, we remove the query term itself in the
%context and the duplicated text.
%During the online processing, the concepts around the target image as well as
...
%to scan the whole web page %to search for the query term and its windows
...
%and the query. Besides these we also apply a list structure detection technique,
...
%First we search for all the occurrences of the query string in the document.
