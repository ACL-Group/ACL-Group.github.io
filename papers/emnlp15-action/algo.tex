...
We show the framework of our system in \figref{fig:system}.
...
%accuracy of dependency parser would be worse.
...
%The key of this algorithm is to prune the subtrees
%if the path from the root to them already forms an invalid solution (not a clique or size is larger than k),
%or if the current path does not have the potential to produce a better solution
%than the current best. To check whether the current
%path could have a better solution, we traverse the search space
%in the descending order of concept weights $w_v(c)$. Suppose
%the number of selected concepts in the current path is $ck$ and
%a decision needs to be made on whether to include concept $c$, we
%compute the upper bound weight for the current path
%by choosing the remaining $k-ck$ concepts with the largest scores.
%If the upper bound is not better than the current
%best solution, we prune the subtree rooted at $c$.
...
%It backtracks to exclude $c_i$ when the left branch is searched (Line 20-23).
