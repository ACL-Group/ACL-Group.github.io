...
\textbf{[FES-LM]}~\cite{peng2017joint} trains a language model with frame, 
...
%because Roemmele~\cite{roemmele2017rnn} has shown that training 
...
%\bf{Methods} &SIMP&SKBC &DSSM &CGAN &SeqMANN &GPT &ISCK &GMSA &Ours \\ \hline
%LM Train &BC &ROCS &ROCS &ROCS &ROCS &BC &BC + ROCS&ROCS&ROCS\\ 
%Pred. Train &ROCS*(Tr)& ROCS*(Tr)&-&-&ROCS*(Tr)&ROCS*(Tr)&ROCS*(Tr)&-&ROCS*(Tr) \\ 
%Validation&ROCS*(V)&ROCS*(V)&-&-&ROCS*(V)&ROCS*(V)&ROCS*(V)&-&ROCS*(V)\\
%Test&SCT(T) &SCT(T)&SCT(T)&SCT(T)&SCT(T)&SCT(T)&SCT(T)&SCT(T)&SCT(T)\\ \hline
\bf{Methods} & LM Train & Pred. Train & Validation & Test \\ \hline \hline
SIMP & BC & ROCS*(Tr)& ROCS*(V)& SCT(T) \\ \hline
SKBC & ROCS & ROCS*(Tr)& ROCS*(V)& SCT(T)\\ \hline
DSSM & ROCS & -  & - &  SCT(T)\\ \hline
FES-LM & NYT+ROCS & -  & - &  SCT(T)\\ \hline
CGAN & ROCS & - & - &  SCT(T)\\ \hline
SeqMANN & ROCS & ROCS*(Tr)& ROCS*(V)& SCT(T)\\ \hline
GPT & BC &  ROCS*(Tr)& ROCS*(V)& SCT(T)\\ \hline
ISCK & BC + ROCS & ROCS*(Tr) & ROCS*(V) & SCT(T) \\ \hline
...
The following preliminary experiment shows why. 
...
\subsection{Different Language Models}
\label{sec:lm}
\begin{table}[th!]
\small
\centering
...
\begin{tabular}{lc}
\hline
\textbf{Language Model}&Acc (\%)\\
\hline
GPT~\cite{radford2018improving}&61.84\\
BERT~\cite{devlin2018bert}&63.66\\
Skip-thought~\cite{kiros2015skip}&68.13\\
\hline
\end{tabular}}
...
However, in \tabref{tab:all-models}, 
SKBC with Skip-thought language model performs better. 
In \tabref{tab:lm}, we further show that Skip-thought is 
