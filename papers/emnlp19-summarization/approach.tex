...
and propose our novel repetition reduction method, which is an extension to the basic model discussed in \secref{sec:basic}.
...
In each layer, GLU \cite{DauphinFAG17} and residual connections \cite{HeZRS16} are used respectively as a non-linear 
gating mechanism and guarantee for sufficient depth of the network.  
\begin{equation}
\small
...
\begin{figure*}[th]
	\centering
	\epsfig{file=model.eps, width=1.8\columnwidth}
...
%Thus, attention filter mechanism helps avoid repeatedly attending to the same POIs, and therefore avoid repetition in summary generation.
...
document (Example 1), we propose a sentence-level backtracking decoder at testing.
