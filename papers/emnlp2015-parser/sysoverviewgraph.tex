...
\tikzstyle{input}=[draw, fill=blue!20, text width=5em,
    text centered, minimum height=2em,drop shadow]
\tikzstyle{output}=[draw, fill=green!20, text width=5em,
    text centered, minimum height=2em,drop shadow]
\tikzstyle{intermediate}=[draw,dashed, fill=blue!20, text width=5em,
    text centered, minimum height=2em,drop shadow]
\tikzstyle{sys} = [draw, text width=6em, fill=red!20,text centered,
...
\begin{tikzpicture}
    ...
    \path (oracle.east)+(3,0) node (seqpredictor) [sys] {Sequence\\Predictor};
    \path (trainset.east)+(3,0) node (headmapper) [sys] {Head\\Mapper};
    \path (oracle.east)+(7,0) node(testset) [input] {Test\\Sentence};
    \path (trainset.east)+(7,0) node(output) [output] {Output\\Parse};

    \path [draw, ->,line width=0.8pt] (trainset.north) --  node[right] {} (oracle.south);
    ...
    \path [draw, ->,line width=0.8pt] (testset.west)+(0,-0.1) --  node[left=0.5,below=0.5,rotate=47] {\textcircled{\small{5}} query} (temp2);
    \path [draw, ->,line width=0.8pt] (headmapper.east) --  node[below] {\textcircled{\small{6}} decode} (output.west);
    \path (headmapper.south)+(4,-0.5) node(parse) {\textbf{Parse Process}};
    \path (seqpredictor.north)+(-4,0.5) node(train) {\textbf{Train Process}};

    \node at (1.1,-1.3) {\textcircled{\small{1}} preprocess};
