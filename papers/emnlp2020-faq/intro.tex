...
Given a context, it is possible to generate many questions and answers.
...
%For further researches in the future, we provide two different frameworks as baselines for this new task. 
%The first one is naive and intuitive, which consists of three parts: answer extraction, question generation, and a filter. 
%We generate the QA pair from the first two steps and filter out those that are not related to the aspect. We call it ``Filter Methods Baseline(FMB)''. However, this baseline relies heavily on the accuracy of the filter and does not integrate the aspect into the task well. To solve the problem, we provide another strong baseline by adding aspect as one of the inputs in the first two steps. We call the second baseline ``Aspect-based Methods Baseline (AMB)''. The second baseline outperforms the first baseline by a large margin on our test set.

%To generate the QA pairs based on the context and aspect keywords,
...
%We generate the QA pairs from the first two steps, then filter out those are not related to the aspect keyword.
...
To evaluate the performance of above methods, we design an new automatic metric which considers both the quality of generated questions and answers. 
