...
\algoref{alg:dfs} shows the pseudo code of this step.
...
%Our goal is to fill the priority queue $Q$ (with budget $B$) with relatively higher quality
...
%monotonicity property to prune the search space.
...
%We thus maintain multiple priority queues sharing a sum budget, one for each skeleton.
The size of each queue is proportional to the support of the corresponding
skeleton.
%\KZ{Is there a bound on the number of skeletons generated from the BFS?}
