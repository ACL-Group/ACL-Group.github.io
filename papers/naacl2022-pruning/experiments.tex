...
\section{Experiments}
We present our pruning setup and detailed analysis in \secref{sec:LAMA}. Then we compare the knowledge transfer ability of pruned subnetworks against original PLMs in scenarios requiring knowledge of single or multiple relations for reasoning in \secref{sec:ckbc} and \secref{sec:csr} respectively.
...
a ratio of 7:2:1. Detailed statistics are listed in \tabref{table:conceptnet}. 
...
\figref{fig:both}~(right) shows that (i) increasing the number of pruned layers helps distill more knowledge; and
...
As indicated by the green curve in the left part of \figref{fig:both}, 
...
the P@1 drops drastically to 0.4 (red curve in the left part of \figref{fig:both}). 
...
\noindent
\textbf{Test set results.}~~\tabref{table:rank} summarizes the test set results. 
...
was given earlier in \figref{fig:LAMA}, and now we show more examples of 
other ConcetpNet relations in \figref{fig:attention}. We observe that while the original 
...
\tabref{table:linkprediction} shows most of the supervised 
...
derive a new relational knowledge triple. For example, the two triples \triple{s1, IsA, o1} and 
\triple{s2, HasProperty, o2}, where o1 equals to s2, can be combined into 
a new test triple \triple{s1, HasProperty, o2}. Based on this rule, 
...
of pruned subnetworks with original PLMs. \tabref{table:twohop} shows that pruned subnetworks 
...
\begin{tabular}{l|c|c@{}}
\toprule
\textbf{Model}              & \multicolumn{1}{l|}{\textless{}~linen, IsA, ?~\textgreater{}}              & \multicolumn{1}{l}{\textless{}~sing, Causes, ?~\textgreater{}}     \\ \midrule
DistilBERT         & \begin{tabular}[c]{@{}c@{}}surname\\ \underline{commodity}\\ profession\end{tabular} & \begin{tabular}[c]{@{}c@{}}death\\ disease\\ trouble\end{tabular} \\ \midrule
DistilBERT(pruned) & \begin{tabular}[c]{@{}c@{}}{\color[HTML]{32CB00}cloth}\\ \underline{fabric}\\ \underline{garment}\end{tabular}         & \begin{tabular}[c]{@{}c@{}}{\color[HTML]{32CB00}happiness}\\ \underline{joy}\\ \underline{peace}\end{tabular}   \\ \bottomrule
\end{tabular}
\caption{Top-3 predictions of two triples sampled from CKBC dev set. The predictions are ranked by probability in descending order with ground-truth marked in green and other plausible answers underlined.}
\label{table:casestudy}
\end{table}

\textbf{Case Study.}~~ We present a case study of link prediction examples from both pre-trained and 
pruned DistilBERT in \tabref{table:casestudy}.
In both examples, the pre-trained DistilBERT makes completely unrelated predictions~(e.g.,\textit{surname}, \textit{profession}, and \textit{death}) and only two predictions~(i.e., \textit{commodity} and \textit{trouble}) can be hardly considered as plausible. In contrast, the model pruned for
IsA/Causes are specialized in accurately representing these relations and can even produce reasonable answers in addition to the ground-truth.
...
\textbf{Stand-alone fine-tuning.}~~We conduct stand-alone fine-tuning using 
...
\begin{figure}[th!]
	\centering
	\scalebox{0.7}{\includegraphics[width=1.0\columnwidth]{figure/copa.png}}
...
It can be observed from \tabref{table:zeroshot} that the pruned models can actually surpass their full-size
