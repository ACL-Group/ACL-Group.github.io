...
\PC{We mentioned that our ambition is to suppress items as few as possible,
meanwhile we also want to preserve the data distribution and prevent spurious
rules in mining association rules. By noticing the reason why global
suppression will not introduce spurious rules, we introduce a heuristic
function to imitate global suppression in order to avoid introducing spurious
rules. However, this concept goes the opposite direction for preserving data
distribution. Therefore, we should introduce the other heuristic function
separately to preserve data distribution.
%The reason for introducing two heuristic functions is that we suppose that these two applications are both important,
%most previous work focuses on the data mining tasks but ignores the most
%simple and effective statistic method.
}

%Heuristic
\XH{ Should we define the notation: Inference/rule, confidence, sensitive
association inference/rule, violated/safe/unsafe sensitive association
inference?}
 \PC { We first present the heuristic function which helps to preserve
data distribution. Consider the inference$\mathcal{R}(q,e)$where
$e$ is a sensitive item and $P(e|q) > \rho$, $q$ is not
a safe $qid$ following the Definition \ref{def:safety_qid}. To make $q$ safe
we must lower the confidence of the inference not bigger than $\rho$, thus we
choose an item type $i$ in $\mathcal{R}(q,e)$ and decide the minimum number
of occurrences of $i$ to be deleted from $\container(q\cup\{e\})$ or
just eliminate this inference from $T$ (for the simplicity we don't consider
the differences created by the removal from different records in
$\container(q\cup\{e\})$).
 %Suppose $C(i, $\mathcal{R}(q,e)$)$ \XH{Kenny name it?}be the
%number of suppression of item $i$ to fix\XH{??} $q\cup\{e\}$.
Relative entropy is defined as $H(Q(i)||P(i))=Q(i)log\frac{Q(i)}{P(i)}$ where
$P(i)$ is the original distribution of item $i$ and $Q(i)$ is the current
(before selecting this removal) distribution of item $i$. We define
$HEUR_{dist}=$ $\frac{H(Q(i)||P(i))}{C(i, \mathcal{R}(q,e))}$ as the
heuristic function which helps to preserve data distribution. The ratio
expresses the strategy that picking an item type $i$ to suppress which
maximally recovers the data distribution and minimizes the deletion.
 The numerator indicates the divergence of the data distribution. The larger the number is,
the larger distribution difference of item type $i$ is compared with the
original dataset. So the removal of $i$ is urgent and valuable. The
denominator indicates the suppression number. The smaller the number is, the
fewer items are suppressed. Each time we choose the violated sensitive
inference \XH{Define sensitive inference?} with highest $HEUR_{dist}$ value
to perform the deletion in priority. }

\PC { Then we introduce our second heuristic function which prevents
introducing spurious rules in mining association rules. As we mentioned
before, to learn from global suppression, we aim at devising a heuristic
which take deletions towards the global suppression effect in priority.
We define $HEUR_{mine}=remain(i)*C(i, \mathcal{R}(q,e))$ as
the heuristic function which prevents introducing spurious rules  in mining
association rules. This product expresses the strategy that we want to
introduce fewer spurious rules by imitating the effect of global suppression
and suppressing as few items as possible. As you can see, each time we choose
the violated sensitive inference \XH{Define sensitive inference?}\PC{What is sensitive inference?} with lowest
$HEUR_{mine}$ value to perform the deletion in priority.}
