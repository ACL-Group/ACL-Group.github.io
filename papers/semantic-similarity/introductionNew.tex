...
can be modeled by term co-occurrence). % In this paper, we are concerned
...
Recent work on term similarity can be roughly classified into two main
...
engines. % Unfortunately, these mechanisms are often ``commercial
...
\begin{itemize}
\item High similarity: \pair{general~ electric}{ge}\\
  Synonyms that refer to the same entity should have the highest
  similarity score.
\item High similarity: \pair{microsoft}{apple},
  \pair{orange}{red} \\
...
\item Low similarity: \pair{music}{lunch}, \pair{banana}{garlic} \\
...
\item Low similarity: \pair{apple}{ipad}, \pair{car}{journey}\\
...
The rest of the paper is organized as follows. \secref{sec:knowledge} introduces
the preliminaries of Probase, our isA semantic network.
\secref{sec:basic} describes a basic algorithm for computing term similarity
using Probase. \secref{sec:refine} then proposes an important refinement to
the basic algorithm which addresses several key challenges faced by the basic
approach.
\secref{sec:eval} gives some experimental results that compare our approach with
a whole list of other previous approaches both using knoweldge and using external
corpora. Finally we discuss some related work in \secref{sec:related} and
conclude in \secref{sec:conclude}.
