...
the solution or not. \GY{An example of the decision tree
is shown in \figref{fig:search_tree}, in which we take the concept graph in \figref{fig:graph_model} as an example.
If we constrain $k=3$,
then the concept set $\{c_0,c_1,c_2\}$ can be a potential solution.
The symbol $d_i$
means the binary decision on concept $c_i$.
