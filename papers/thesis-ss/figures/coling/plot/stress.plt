...
dataname = "stress_roberta"

# 填充样式数组 - 这里使用整数来表示pattern的类型
array FillStyleArray[10] = [1, 1, 7, 3, 2, 9, 7, 3,2, 9]

array Categories[12] = ["Antonym", "Antonym(mm)", "Negation", "Negation(mm)", "LengthMismatch", "LengthMismatch(mm)", "SpellingError", "SpellingError(mm)", "WordOverlap", "WordOverlap(mm)", "NumericalReasoning", "Average"]
...
do for [j=1:12] {
    ...
    set output sprintf("%s_%s.png", dataname, Categories[j])
    
    # Fetch the original value and compute maximum value for setting yrange
    stats dataname.'.dat' using 2 every ::j::j nooutput
    print "Processing line: ", j, " Category: ", Categories[j],max_val
    #stats 'stress_bert.dat' using 2 every ::j::j nooutput
    original_val = STATS_mean

    max_val = original_val * 1.1
    do for [i=2:11] {
    stats dataname.'.dat' every ::(j)::(j) using i nooutput
    ...
    plot for [i=2:3] dataname.'.dat' u ((i-1)*0.5):i every ::(j-1)::(j-1) w boxes title columnheader(i) lc rgb ColorArray[i-1] fillstyle solid noborder,\
    for [i=4:11] dataname.'.dat' u ((i-1)*0.5):i every ::(j-1)::(j-1) w boxes title columnheader(i) lc rgb ColorArray[i-1] fillstyle pattern FillStyleArray[i-1] noborder,\
