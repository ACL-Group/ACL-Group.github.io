...
%General framework of dataset collection is shown in Figure \ref{fig:framework}.\JQ{和下边标题没有对应}
...
Each instance in our CoCon dataset is %composed of 
%two short phrases and one label
a triple: $\{p_1, p_2, l\}$,
%Formally, each instance in our CoCon dataset is composed of 2 short phrases and 4 options: $\{p_1, p_2, r_1, r_2, r_3, r_4\}$. 
...
%For the more implausible one, four options $\{r_1, r_2, r_3, r_4\}$ are offered to be chosen as the reason of being implausible. For both tasks, we use accuracy score to do the evaluation.
...
We use BERT to calculate perplexity (see Equation \ref{ppl}) of each phrase and those which have lower perplexity score in each pair are considered as plausible one. The accuracy of BERT achieves 90.6\%. However, after viewing all generated samples,
...
\KZ{Again? The most natural and naive way} is to directly generate associated similar positive samples starting with those negative annotated ones. %The following are our generation steps:
...
We first crawled user queries on e-commerce platform with data masking among 7 days (2019.07.15-2019.07.21). %After filtering those queries that only have one word and 
...
%\KZ{You haven't introduced the net at all yet.}
...
%After constructing net, 
After construction, the weight of each edge in the graph represents the frequency %number of times 
...
is less plausible. %The reason is that there are lots of female users search for \textit{dress} so even the strange combination of words may still appear much more times than a normal collocation with \textit{torch}. 
...
%Besides, as there are countless combination of words, for example, the combination of \textit{color} and most daily things are plausible, but the user query in 7 days may not cover all. Hence, we make another assumption after observing the result: Pair never appear $\neq$ implausible, BUT rare appear $=$ implausible. This assumption is a little strong but efficient for us to constrain the scope of positive samples.
...
%Besides, as most positive samples are common enough, 
...
%Therefore, we apply 
Verifying the validity of \textit{Concept Association Graph}%the constructed graph
, the above method is applied on 
our whole unlabeled preprocessed 4,081,254 phrases, %\KZ{pairs?}, 
top 15\% (1,176,922 phrases) by overall plausibility score are kept. 
%we fetch the previous 15\% (equals to 176,538 samples) for further human annotation.
...
This part describes the guideline for annotators: when to ignore a phrase and what kind of implausible phrases do we need. %that the annotators are given to follow.
...
%Given a short phrase, annotator first choose if it should be ignored. Then 
\begin{enumerate}
	\item Phrases that belong to any one of these four cases can be directly ignored%\JQ{deleted}: 
	%satisfies 
	\begin{itemize}
		\item [-] plausible phrase. %Correct commonsense phrase.
		\item [-] Absence of a clear subject.%, such as "children deer''(儿童麋鹿). %Not clear and can not distinguish the subject in a phrase, such as "children deer''(儿童麋鹿).\JQ{这条没懂}
	...
	\begin{itemize}
		...
		\item [-] The modifier is contradictory with the intrinsic property of subject. For example, ``vitreous porcelain''(玻璃青花瓷), %``thermal ice bag''(保温冰袋), 
		``children sexy dress''(儿童性感连衣裙).%, ``boy night skirt''(男童睡裙).
...
We distribute these 18,000 fetched phrases %\KZ{``data" is uncountable noun! Either is pairs or phrases.} 
to 6 person,% who is asked 
...
Answers from annotators who choose correctly on more than 80\% checking problems will be counted into the final collection. Figure \ref{fig:crowd} is the crowdsourcing web interface of data verification.%\JQ{right?}
...
During validation, each sample is judged by 5 person, and those pairs that gain more than 4 same votes are kept. %After this step, 
